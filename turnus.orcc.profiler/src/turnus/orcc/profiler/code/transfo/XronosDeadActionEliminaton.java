 ...
 * 
 */
public class XronosDeadActionEliminaton extends DfVisitor<Void> {
	...
	public XronosDeadActionEliminaton() {
		this(false);
	}

	public XronosDeadActionEliminaton(Boolean debug) {
		this.debug = debug;
	...
	public Void caseActor(Actor actor) {
		...
		for (Action action : actor.getActions()) {
			...
			if (ValueUtil.isBool(eliminate)) {
				...
				if (!value) {
					if (debug) {
						OrccLogger.warnln("Xronos Tranformation: action \""
								+ action.getName()
