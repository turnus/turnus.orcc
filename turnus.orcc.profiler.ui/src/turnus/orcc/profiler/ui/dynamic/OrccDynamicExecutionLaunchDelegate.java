...
import turnus.orcc.profiler.dynamic.execution.OrccDynamicExecution;
 ...
 *
 */
public class OrccDynamicExecutionLaunchDelegate implements ILaunchConfigurationDelegate {

	@Override
	public void launch(ILaunchConfiguration launchConfiguration, String mode, ILaunch launch, IProgressMonitor monitor)
			throws CoreException {
		try {
			monitor.subTask("Turnus Dynamic Execution Analysis");

			final Configuration configuration = parse(launchConfiguration, mode);
			// build the profiling job
			Job job = new Job("TURNUS Orcc Dynamic Execution Profiler") {

				private OrccDynamicExecution profiler;
				...
				protected IStatus run(IProgressMonitor monitor) {
					try {
						...
						profiler = new OrccDynamicExecution();
			...
			// configure process
			launch.addProcess(new TurnusProcess("dynamic execution analysis", job, launch));
