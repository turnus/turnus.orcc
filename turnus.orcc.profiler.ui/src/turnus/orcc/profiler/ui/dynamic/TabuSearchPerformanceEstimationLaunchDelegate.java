...
import turnus.orcc.profiler.dynamic.numa.partitioning.TabuSearchPerformanceEstimation;
 ...
 *
 */
public class TabuSearchPerformanceEstimationLaunchDelegate implements ILaunchConfigurationDelegate {
	...
	public void launch(ILaunchConfiguration launchConfiguration, String mode, ILaunch launch, IProgressMonitor monitor)
		...
		try {
			monitor.subTask("TURNUS tabu search (NUMA + performance estimation)");

			final Configuration configuration = parse(launchConfiguration, mode);
			// build the profiling job
			Job job = new Job("TURNUS tabu search (NUMA + performance estimation)") {

				private TabuSearchPerformanceEstimation tabuSearch;
				...
				protected IStatus run(IProgressMonitor monitor) {
					try {
						...
						tabuSearch = new TabuSearchPerformanceEstimation();
						tabuSearch.run(configuration);
			...
			// configure process
			launch.addProcess(new TurnusProcess("TURNUS tabu search (NUMA + performance estimation)", job, launch));
