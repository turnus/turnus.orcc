...
public class BasicOptionsTab extends AbstractLaunchTab {
	...
	public BasicOptionsTab() {
		super("Basic Options", Icon.APPLICATION_ICON);
	}
	...
	protected void createOptionWidgets(Composite composite) {
		{ // Orcc Project Configuration Options
			...
			w = new LaunchWidgetSelectFileOptional(CAL_STIMULUS_FILE, "*", null, group);
			w.setText("Input Stimulus (optional)");
			addWidget(w);
			
			w = new LaunchWidgetSelectPathOptional(SIMULATION_OUTPUT_FILE, false, null, group);
			w.setText("Output file (optional)");
			addWidget(w);
		}
