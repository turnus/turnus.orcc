...
import turnus.orcc.profiler.dynamic.interpreter.OrccDynamicInterpreter;
 ...
 *
 */
public class OrccDynamicInterpreterLaunchDelegate implements ILaunchConfigurationDelegate {

	@Override
	public void launch(ILaunchConfiguration launchConfiguration, String mode, ILaunch launch, IProgressMonitor monitor)
			throws CoreException {
		try {
			monitor.subTask("Turnus Dynamic Interpretation Analysis");

			final Configuration configuration = parse(launchConfiguration, mode);

			// build the profiling job
			Job job = new Job("TURNUS Orcc Dynamic Interpreter Profiler") {

				private OrccDynamicInterpreter profiler;
				...
				protected IStatus run(IProgressMonitor monitor) {
					try {
						...
						profiler = new OrccDynamicInterpreter();
			...
			// configure process
			launch.addProcess(new TurnusProcess("dynamic interpretation analysis", job, launch));
